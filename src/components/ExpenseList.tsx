  ...
  if (expenses.length === 0) {
    return (
      <div className="neo-bg p-10 rounded-3xl neo-shadow text-center mt-2 flex flex-col items-center justify-center">
        ...
        <h2 className="text-xl font-bold text-slate-700 dark:text-slate-100 mb-2 tracking-tight">No transactions yet</h2>
    ...
    <div className="neo-bg rounded-3xl neo-shadow flex flex-col mt-2 overflow-hidden">
      <div className="p-5 border-b border-slate-200/50 dark:border-slate-700/50 flex flex-col gap-4 neo-bg z-10 sticky top-0">
        <div className="flex justify-between items-center">
          <h3 className="font-bold text-slate-700 dark:text-slate-100 text-lg">Transactions</h3>
            ...
            <AnimatePresence>
              {isExportMenuOpen && (
                <>
                  ...
                  >
                    <button onClick={handleExportPDF} className="w-full text-left px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-100 hover:neo-inner transition-colors">
                      PDF Document
                    </button>
                    <button onClick={handleExportExcel} className="w-full text-left px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-100 hover:neo-inner transition-colors">
                      Excel file
                    </button>
                  </motion.div>
                </>
        ...
        <div className="flex gap-2">
          <div className="relative group flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 neo-bg neo-inner rounded-xl text-sm outline-none transition-all text-slate-700 dark:text-slate-100 placeholder:text-slate-400 dark:text-slate-400 focus:text-amber-600"
            />
          </div>
          <div className="w-1/3">
            <select
              ...
              className="w-full px-3 py-2.5 neo-bg neo-shadow-sm rounded-xl text-sm outline-none transition-all appearance-none text-slate-700 dark:text-slate-100 font-medium active:neo-inner-sm focus:neo-inner"
            >
              {categories.map((cat) => (
          ...
          {sortedExpenses.length > 0 ? (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-100 mb-0.5 flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                <span className={`font-bold text-sm tracking-tight ${expense.type === "income" ? "text-emerald-500" : "text-slate-700 dark:text-slate-100"}`}>
