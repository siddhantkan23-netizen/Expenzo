...
export default function Auth({ onLogin }: AuthProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full relative overflow-hidden neo-bg">
      ...
      <div className="absolute top-8 left-6 z-10 flex items-center gap-2">
         <button className="w-10 h-10 neo-bg rounded-full flex items-center justify-center neo-shadow active:neo-inner text-slate-500 dark:text-slate-300 transition-all">
           <ChevronLeft className="w-5 h-5" />
         </button>
         <span className="text-slate-500 dark:text-slate-300 font-bold text-sm hidden sm:inline">Back</span>
      </div>
      ...
      <div className="w-full md:w-1/2 flex items-center p-0 md:p-8">
        ...
        >
          ...
          <div className="flex bg-slate-200/50 dark:bg-slate-800/50 neo-inner rounded-2xl p-1 mb-8 max-w-sm mx-auto">
            <button
              type="button"
              onClick={() => {
                setIsLogin(true);
          ...
          <form onSubmit={handleSubmit} className="space-y-6 max-w-sm mx-auto">
            {!isLogin && (
              <div>
                <label className="block text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Full Name</label>
                <input
                  type="text"
                  value={name}
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Email</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Password</label>
              <input
                type="password"
                required
                value={password}
            ...
            <div className="flex items-center justify-between text-sm px-1">
               <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setRememberMe(!rememberMe)}>
                  ...
                  <span className="text-slate-500 dark:text-slate-300 font-medium select-none">{isLogin ? "Remember me" : "I agree to the processing of Personal data"}</span>
               </label>
               {isLogin && (
